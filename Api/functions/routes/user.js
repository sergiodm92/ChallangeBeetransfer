...
const { authRegister, authLogin } = require("../services/user.service");
...
route.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (token.length > 100) {
      ...
      res
        .status(400)
        .json(customResponseError("Could not login, please try again..."));
    }
  } catch (error) {
...
route.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      if (user == false)
        res.status(400).json(customResponseError("user already exists"));
      else
        res
          .status(400)
          .json(customResponseError("Failed to register, please try again..."));
    }
  } catch (error) {

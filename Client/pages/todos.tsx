...
export default function Todos() {
      ...
      .then((response) => {
        setCopyTodos(
          response.data.data.filter(
            (t: { delete: boolean }) => t.delete === false
          )
        );
        setTodosDeleted(
          response.data.data.filter(
            (t: { delete: boolean }) => t.delete === true
          )
        );
      })
      .catch((error) => {
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        ...
        axios
          .put(
            `${URL}/todo/edit`,
            { id: id, title: newTitle, text: newText },
            {
              headers: {
                "auth-token": `${sessionStorage.token}`,
              },
  ...
  const handleChangeFilter = (e: SelectChangeEvent) => {
    ...
    if (e.target.value == "Completadas") {
      setTodos(
        copyTodos.filter((a: { completed: boolean }) => a.completed == true)
      );
    }
    if (e.target.value == "Pendientes") {
      setTodos(
        copyTodos.filter((a: { completed: boolean }) => a.completed == false)
      );
    }
    if (e.target.value == "Eliminadas") {
      setTodos(todosDeleted);
    }
    if (e.target.value == "a-z") {
      setSort(e.target.value);
      setTodos(
        copyTodos.sort(function (a: { title: string }, b: { title: string }) {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.filters}>
        <div className={styles.selects}>
          ...
          >
            <MenuItem sx={{ fontSize: "12px" }} value={"Todas"}>
              Todas
            </MenuItem>
            <MenuItem sx={{ fontSize: "12px" }} value={"Completadas"}>
              Completadas
            </MenuItem>
            <MenuItem sx={{ fontSize: "12px" }} value={"Pendientes"}>
              Pendientes
            </MenuItem>
            <MenuItem sx={{ fontSize: "12px" }} value={"Eliminadas"}>
              Eliminadas
            </MenuItem>
          </Select>
        </div>
        <div className={styles.selects}>
          ...
          <Select
            ...
            onChange={handleChangeFilter}
          >
            <MenuItem sx={{ fontSize: "12px" }} value={"a-z"}>
              Nombre A-Z
            </MenuItem>
          </Select>
        </div>
